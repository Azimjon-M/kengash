...
import azolarApi from "../../services/azolar";
...
const AzolarCom = () => {
  ...
  useEffect(() => {
    azolarApi.get()
      .then((response) =>
        setData(response.data.filter((obj) => obj.lavozim !== "superadmin"))
      )
    ...
    <div className="bg-[#F3F7FA] min-h-[calc(100vh-125px)]">
      ...
      <div className="px-5">
        <div className="flex items-center justify-between max-w-7xl mx-auto mb-5">
          ...
          <Link
            to="/azolar/addAzo"
            className="btn bg-[#05B967] hover:bg-[#07b867] text-white"
          >
            A'zo qo'shish
          </Link>
        </div>
                  ...
                  <tbody className="text-sm lg:text-base">
                    {data.map((user, key) => {
                      return (
                        ...
                        >
                          <td className="whitespace-nowrap font-semibold px-2 py-3">
                            {user.id}
                          </td>
                          <td className="whitespace-nowrap font-medium px-2 py-3">
                            {user.username}
                          </td>
                          <td className="whitespace-nowrap font-medium px-2 py-3">
                            {user.first_name}
                          ...
                          <td className="whitespace-nowrap font-medium py-3 flex justify-center">
                            <button
                              onClick={() => handleDelete(user.id)}
                              className="btnDelete"
                            >
                              <MdDeleteOutline className="text-red-700 cursor-pointer w-5 md:w-6 h-auto" />
                            </button>
                          </td>
                        </tr>
                      );
